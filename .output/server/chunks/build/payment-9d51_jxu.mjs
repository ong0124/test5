...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(` ${ssrInterpolate(_ctx.$t("paymentPage.OrderTotal"))}</h3><div class="space-y-3"><div class="flex justify-between text-gray-600"><span>${ssrInterpolate(_ctx.$t("paymentPage.OrderAmount"))}</span><span>NT$ ${ssrInterpolate(unref(totalprice))}</span></div><div class="flex justify-between text-gray-600"><span>\u670D\u52A1\u7A0E</span><span>NT$ 0.00</span></div><div class="border-t border-gray-200 my-2"></div><div class="flex justify-between font-semibold text-gray-600"><span>${ssrInterpolate(_ctx.$t("paymentPage.total"))}</span><span class="text-green-600">NT$ ${ssrInterpolate(unref(totalprice))}</span></div></div></div></div><div class="sticky bottom-12 bg-white py-4 px-6 shadow-lg border-t border-gray-100"><button class="w-full py-3 px-6 bg-gradient-to-r from-green-500 to-green-600 text-white font-medium rounded-lg text-center shadow-md hover:shadow-lg transform hover:scale-[1.02] transition-all duration-300 flex items-center justify-center">`);
      _push(ssrRenderComponent(_component_Icon, {
        name: "mdi:tick",
        class: "w-5 h-5 text-white mr-2"
      }, null, _parent));
      _push(` ${ssrInterpolate(_ctx.$t("paymentPage.pay_button"))} (NT$ ${ssrInterpolate(unref(totalprice))}) </button></div></div>`);
    };
  }
});
...
//# sourceMappingURL=payment-9d51_jxu.mjs.map
