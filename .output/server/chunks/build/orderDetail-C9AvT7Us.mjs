...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</div><h1 class="absolute left-1/2 transform -translate-x-1/2 text-xl text-white font-bold capitalize">${ssrInterpolate(_ctx.$t("orderDetail.title"))}</h1></header><div class="px-6 pt-4"><div class="bg-amber-50 py-4 px-2 rounded-t-xl relative overflow-hidden border-b border-gray-300"><div class="absolute top-2/2 left-2 -translate-y-1/2 w-4 h-4 bg-gray-200 rounded-full"></div><div class="absolute top-2/2 right-2 -translate-y-1/2 w-4 h-4 bg-gray-200 rounded-full"></div><div class="flex items-center justify-between"><div class="flex-1 text-gray-600 text-lg text-end pr-6"><p>${ssrInterpolate(("Translate_loc" in _ctx ? _ctx.Translate_loc : unref(Translate_loc))(unref(form).departure_loc))}</p></div><div class="flex-2 text-center text-gray-600"><div class="border-b-2 border-dashed border-lwm-50 mx-2 pb-1 text-md font-medium"><p>${ssrInterpolate(("formatDate" in _ctx ? _ctx.formatDate : unref(formatDate))(unref(form).shuttle_date))}</p></div><div class="text-sm text-gray-600"><p>${ssrInterpolate(unref(form).shuttle_time)}</p></div></div><div class="flex-1 text-gray-600 text-lg text-start pl-6"><p>${ssrInterpolate(("Translate_loc" in _ctx ? _ctx.Translate_loc : unref(Translate_loc))(unref(form).destination_loc))}</p></div></div></div>`);
      if (unref(form).trip_type === "roundTrip") {
        _push(`<div class="bg-amber-50 py-4 px-2 relative overflow-hidden border-b border-gray-500"><div class="flex items-center justify-between"><div class="flex-1 text-gray-600 text-lg text-end pr-6"><p>${ssrInterpolate(("Translate_loc" in _ctx ? _ctx.Translate_loc : unref(Translate_loc))(unref(form).return_departure))}</p></div><div class="flex-2 text-center text-gray-600"><div class="border-b-2 border-dashed border-lwm-50 mx-2 pb-1 text-md font-medium"><p>${ssrInterpolate(("formatDate" in _ctx ? _ctx.formatDate : unref(formatDate))(unref(form).return_shuttle_date))}</p></div><div class="text-sm text-gray-600"><p>${ssrInterpolate(unref(form).shuttle_time)}</p></div></div><div class="flex-1 text-gray-600 text-lg text-start pl-6"><p>${ssrInterpolate(("Translate_loc" in _ctx ? _ctx.Translate_loc : unref(Translate_loc))(unref(form).return_destination))}</p></div></div></div>`);
      } else {
        _push(`<!---->`);
      }
...
//# sourceMappingURL=orderDetail-C9AvT7Us.mjs.map
