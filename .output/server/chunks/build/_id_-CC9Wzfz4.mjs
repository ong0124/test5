...
import { c as useI18n, a as useRouter } from './server.mjs';
import '@iconify/vue';
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(unref(notTraveled), (index) => {
        ...
        _push(`<p class="w-32 truncate">${ssrInterpolate(_ctx.$t("Booking.arrivalPortTime"))}</p><p class="text-gray-800">${ssrInterpolate(("formatDate" in _ctx ? _ctx.formatDate : unref(formatDate))(index.arrivalpoint_date))}</p><p class="pl-2 font-semibold text-gray-900">${ssrInterpolate(index.arrivalpoint_time)}</p></div></div><div class="text-gray-500"><div class="flex items-center px-2 rounded-md mt-2 space-x-2">`);
        _push(ssrRenderComponent(_component_Icon, {
          name: "mdi:bus-clock",
          class: "w-5 h-5 text-lwm-100"
        }, null, _parent));
        _push(`<p class="w-32 truncate" title="{{ $t(&#39;Booking.shuttleBusTime&#39;) }}">${ssrInterpolate(_ctx.$t("Booking.shuttleBusTime"))}</p><p class="text-gray-800">${ssrInterpolate(("formatDate" in _ctx ? _ctx.formatDate : unref(formatDate))(index.shuttle_date))}</p><p class="pl-2 font-semibold text-gray-700">${ssrInterpolate(index.shuttle_time)}</p></div></div></div>`);
        ...
        if (index.trip_type === "roundTrip") {
          _push(`<div class="text-md text-gray-700"><div class="text-gray-500 flex"><div class="flex items-center px-2 mt-2 space-x-2">`);
          _push(ssrRenderComponent(_component_Icon, {
            name: "i-material-symbols-flight-land",
            class: "w-5 h-5 text-lwm-50"
          }, null, _parent));
          _push(`<p class="w-32 truncate">${ssrInterpolate(_ctx.$t("Booking.arrivalAtAirportTime"))}</p><p class="text-gray-800">${ssrInterpolate(("formatDate" in _ctx ? _ctx.formatDate : unref(formatDate))(index.arrivalpoint_date))}</p><p class="pl-2 font-semibold text-gray-900">${ssrInterpolate(index.flight_time)}</p></div></div><div class="text-gray-500"><div class="flex items-center px-2 rounded-md mt-2 space-x-2">`);
          _push(ssrRenderComponent(_component_Icon, {
            name: "mdi:bus-clock",
            class: "w-5 h-5 text-lwm-100"
          }, null, _parent));
          _push(`<p class="w-32 truncate" title="{{ $t(&#39;Booking.shuttleBusTime&#39;) }}">${ssrInterpolate(_ctx.$t("Booking.shuttleBusTime"))}</p><p class="text-gray-800">${ssrInterpolate(("formatDate" in _ctx ? _ctx.formatDate : unref(formatDate))(index.return_shuttle_date))}</p><p class="pl-2 font-semibold text-gray-700">${ssrInterpolate(index.return_shuttle_time)}</p></div></div></div>`);
        } else {
          _push(`<!---->`);
        }
...
//# sourceMappingURL=_id_-CC9Wzfz4.mjs.map
